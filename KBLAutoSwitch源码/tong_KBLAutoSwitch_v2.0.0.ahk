/* tong（【自动切换输入法键盘布局】）
    ...
    作者: tong
    改动: 删了很多我不需要的代码，该脚本长时间没更新，有些地方有bug，比如alt+tab，uwp应用，新开和创建有点重复，编辑器模式我也不太需要，备注不太完整，重新写了备注
    设置方法：无托盘，无菜单，可以依托于RunAny使用，与tong_KBLAutoSwitch.ini文件一同复制到RunAny的RunPlugins文件夹中，RunAny设置脚本自启
			编辑配置文件tong_KBLAutoSwitch.ini，在相应的内容项目下填写内容
	清单：
...
;【改编者信息】	;{
	...
	;-----配置文件
	; 	1.tong_KBLAutoSwitch.ini配置文件中设置相关选项，中文代码和英文代码已默认配置好，可使用tong_get_keyboard_vx.x.x.ahk获取键盘布局代码，目前微软自带英文键盘和搜狗输入法使用没有问题
	;	2.未设置应用默认为中文输入法，因此中文窗口选项可以不设置，只需将需要切换英文的应用进行设置即可
	;	3.打开AU3_Spy.exe文件，点击对应的窗口可获取对应窗口的ahk_exe，复制添加至tong_KBLAutoSwitch.ini配置文件即可
